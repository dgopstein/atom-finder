...
#define M4(x, y) (x)*(y) // <def-atom> <arg-wrapped>
  !M4(2+3, 4+5);         // <use-atom>
  !(M4(2+3, 4+5));       // <no-atom>

#define M5(x, y) ((x)*(y)) // <no-atom> <arg-wrapped>
  !M5(2+3, 4+5);           // <no-atom>
...
#define M10(x, y) do { /*<no-atom> <multiline> <do-wrapped> <arg-wrapped>*/\
      a = (x)*(y); \
  } while (0)
...
#define M11(x, y) do { /*<def-atom> <multiline> <arg-wrapped>*/\
      a = (x)*(y); \
  } while (0);
