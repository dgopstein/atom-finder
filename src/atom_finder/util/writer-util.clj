...
(defn in-macro-exp?
  "Is this node generated by a macro-expansion"
  [node]
  (->> node .getNodeLocations first (instance? org.eclipse.cdt.core.dom.ast.IASTMacroExpansionLocation)))
...
(defn print-tree [node]
  (pre-tree
   (fn [node index tree-path]
       (printf "%-60s %s\n"
               (str tree-path "  " (write-node-type node) "  " ((if in-macro-exp? write-exp-node write-node-no-type) node))
