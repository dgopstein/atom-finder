...
cve.atom.counts <- data.table(read.csv("data/atom_counts.csv"))
cve.atom.counts$fair.rate <- cve.atom.counts[, -c('X.project', 'X.non.atoms')][, lapply(.SD, function(x) (x/X.all.nodes)/sum(x))][, -c('X.all.nodes')][, .(fair.rate=base::sum(.SD)), by=1:14][, fair.rate]
cve.atom.counts[, atom.rate:=(X.all.nodes-X.non.atoms)/X.all.nodes]

# https://docs.google.com/spreadsheets/u/1/d/1Ey56XWXNn8i6Xypctb8G52iYQ9449g6NfpCf6tpDlKw/edit#gid=0
cve.atom.counts$bug.count <- c(30930,224901,359005,10987,171261,70177,15463,79038,4358,4521,8062,8656,1457,1723)
cve.atom.counts$since <- as.Date(c('2002-11-06','1994-09-14','1998-05-07','1999-08-03','2005-06-07','2009-04-08','2003-10-07','2002-09-12','2003-09-21','2008-02-21','2005-04-29','2001-01-08','2011-08-08','2011-05-15'))
cve.atom.counts[, bug.rate := bug.count/as.numeric(as.Date("2018-01-01")-since)]
cve.atom.counts$domain <- factor(c("os", "os", "browser", "compiler", "browser", "db", "compiler", "db", "vcs", "editor", "vcs", "webserver", "webserver", "editor"),
